...
const checkCourseCode = (rule: any, value: any, callback: any) => {
  ...
  if(/^[0-9]+$/.test(value)){
    return callback(new Error('课程代码不能全是数字'))
  }
  if(/^[A-Za-z]+$/.test(value)){
    return callback(new Error('课程代码不能全是字母'))
  }
  var current_CourseCode;
  var current_CourseName;
  if (dialogFormVisible.value == true) {
        current_CourseCode = ruleForm.CourseCode
        current_CourseName = ruleForm.CourseName
      } else if (dialogFormVisible2.value == true) {
        current_CourseCode = ruleForm2.CourseCode
        current_CourseName = ruleForm2.CourseName
      } else {
  ...
  setTimeout(() => {
    ...
    for (; step < TableCourses.length; step++) {
      ...
      if (TableCourses[step].CourseName == current_CourseName && value != TableCourses[step].CourseCode) {
        // console.log(value) 
...
const rules = reactive({
  ...
  CourseCode: [
    ...
    // { pattern: "^[A-Za-z]+$", message: "课程编号不能只包含字母", trigger: 'blur' },
    // { pattern: "^[0-9]+$", message: "课程编号不能只包含数字", trigger: 'blur' },
  ...
  Teacher: [
    ...
    { pattern: "^[A-Za-z]+$", message: "老师名字只能是英文字母", trigger: 'blur' },
  ]
